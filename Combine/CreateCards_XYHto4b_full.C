...
int main(int argc, char **argv) {
   ...
   string aux_shapes = string(getenv("CMSSW_BASE")) + "/src/CombineHarvester/CombineTools/bin/";
       ...
       if (argc < 2) {
        ...
        cerr << "  mc   -> MC backgrounds with JES/JER/etc systematics" << endl;
        return 1;
    }
  ...
  cats.push_back({1, "h_MX_MY_index_Comb_3_3_2_2_Inclusive"});
  cats.push_back({2, "h_MX_Comb_3_3_2_2_Inclusive"});
  cats.push_back({3, "h_MY_Comb_3_3_2_2_Inclusive"});
  ...
  if (mode == "data_driven") {
      ...
      systNames= {
        "Statistical_Uncertainty", "Systematic_Uncertainty",
      };

      // Add rateParam 
      cb.cp().process({"Inclusive_Bkg"}).AddSyst(
          cb, "bkg_norm", "rateParam", ch::syst::SystMap<>::init(1.0)
      );

      // Set the range [0.1, 2.5] for the parameter
      cb.GetParameter("bkg_norm")->set_range(0.1, 2.5);

      // Apply MY125Bin_Uncertainty exclusively to the MY bin for the background Separately bin 6 
    for (int i = 6; i <= 8; ++i) {
        std::string my_syst_name = "MY125Bin" + std::to_string(i) + "_Uncertainty";
        
        cb.cp()
        .bin({"h_MY_Comb_3_3_2_2_Inclusive"})
    ...
    .bin({"h_MY_Comb_3_3_2_2_Inclusive"})
    .process(ch::JoinStr({bkg_procs}))
    .AddSyst(cb, "NonClosure_Uncertainty", "shape", SystMap<>::init(1.00));


    // Apply the 14 decorrelated Non-Closure uncertainties exclusively to the MX bin for the background
    for (int i = 1; i <= 14; ++i) {
        std::string nc_syst_name = "NonClosure_Bin" + std::to_string(i) + "_Uncertainty";
        
        cb.cp()
        .bin({"h_MX_Comb_3_3_2_2_Inclusive"})
        .process(ch::JoinStr({bkg_procs}))
    ...
    for (int i = 1; i <= 14; ++i) {
        std::string nc_syst_name = "NonClosure_MXBin" + std::to_string(i) + "_Uncertainty";

        cb.cp()
        .bin({"h_MX_MY_index_Comb_3_3_2_2_Inclusive"})
        .process(ch::JoinStr({bkg_procs}))
        .AddSyst(cb, nc_syst_name, "shape", SystMap<>::init(1.00));

        // then add the decorrelated to MX bins affected MY bin 6 7 8
        for (int my_bin = 6; my_bin <= 8; ++my_bin) {
            std::string my_syst_name = "MY125Bin" + std::to_string(my_bin) + "_MXBin" + std::to_string(i) + "_Uncertainty";
            cb.cp()
                .bin({"h_MX_MY_index_Comb_3_3_2_2_Inclusive"})
                .process(ch::JoinStr({bkg_procs}))
                .AddSyst(cb, my_syst_name, "shape", SystMap<>::init(1.00));
...
//  cb.cp().signals()
  ...
  input_filename = "InputFiles/combine_input_XYH4b_"+year+"_VR.root";
...
//   cb.SetAutoMCStats(cb, 0, 1, 1);
  ...
  char filename[100];
  for (auto m : signals) {
	sprintf(filename,"NMSSM-XYHto4b_Par-%s-%s_3bvalidation.input.root",year.c_str(),m.c_str()); 
