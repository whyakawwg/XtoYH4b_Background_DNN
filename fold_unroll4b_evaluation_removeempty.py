...
if args.runType == "test-only":
    ...
    #     valid_mask = (my_indices >= 0) & (my_indices < n_my_bins)
        
    #     unrolled_mx = np.full_like(mx_data, -100.0)
    #     unrolled_mx[valid_mask] = mx_data[valid_mask] + (my_indices[valid_mask] * mx_end)

    #     var_data_map["Unrolled_MX"] = unrolled_mx

    #     unrolled_bins_list = []
    #     for i in range(n_my_bins):
    #         shifted_bins = mx_bins_edge + (i * mx_end)
    #         if i == 0:
    #             unrolled_bins_list.extend(shifted_bins)
    #         else:
    #             if shifted_bins[0] == unrolled_bins_list[-1]:
    #                 unrolled_bins_list.extend(shifted_bins[1:])
    #             else:
    #                 unrolled_bins_list.extend(shifted_bins)
        
    #     unrolled_bins_list = sorted(list(set(unrolled_bins_list)))

    #     binning_map["Unrolled_MX"] = unrolled_bins_list
    ...
    #     h_labels = ROOT.TH1F("Unrolled_Lsabels_Metadata_", "Metadata only", n_valid_bins, 0, n_valid_bins)
    ...
    for var, data in var_data_map.items():
        ...
        for i, fold_num in enumerate(model_metadata):
