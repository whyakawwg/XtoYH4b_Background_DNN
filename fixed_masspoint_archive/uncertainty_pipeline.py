...
TARGET_MY_BINS = [6, 7, 8, 9, 10, 11]         # MY bins with lower edges 90, 110, 135
...
def plot_evaluation(
    ...
    if err_stat_4b is None:
        raise ValueError(
            f"Missing 4b statistical uncertainties for '{var}'. "
            "Pass err_stat_4b when calling plot_evaluation()."
        )
    err_stat_4b = np.asarray(err_stat_4b)
    ...
    ratio_err_4b2b = ratio_4b_2b * np.sqrt(rel_err_num**2 + rel_err_den**2)
...
def run_create_uncertainty_histograms(args):
    ...
    if f_in is None or f_in.IsZombie():
        raise OSError(f"Could not open evaluation input file: {input_file}")

    # Validate every required non-closure histogram before recreating the output
    # file. This prevents a failed Higgs-window run from leaving an MX-only file.
    if not SaveNonClosure:
        f_nc = ROOT.TFile.Open(NonClosureFactor_path, "READ")
        if not f_nc or f_nc.IsZombie():
            f_in.Close()
            raise OSError(f"Could not open non-closure file: {NonClosureFactor_path}")

        validation_errors = []
        for var in vars_to_plot:
            h_source = _get_hist_safe(f_in, f"{var}_hist_4b_mean")
            h_nc = _get_hist_safe(f_nc, f"{var}_nonclosure_factor")
            if h_source is None:
                validation_errors.append(f"missing input histogram '{var}_hist_4b_mean'")
            if h_nc is None:
                validation_errors.append(
                    f"missing non-closure histogram '{var}_nonclosure_factor'"
                )
            if h_source is not None and h_nc is not None:
                if h_source.GetNbinsX() != h_nc.GetNbinsX():
                    validation_errors.append(
                        f"bin mismatch for {var}: input has {h_source.GetNbinsX()} bins, "
                        f"non-closure histogram has {h_nc.GetNbinsX()} bins"
                    )
        ...
        if validation_errors:
            ...
            raise RuntimeError(
                f"Non-closure validation failed for {NonClosureFactor_path}:\n  - {details}\n"
                "Run the test-region create_unc_hists step successfully first."
            )
    ...
    for var in vars_to_plot:
        ...
        if SaveNonClosure:
            ...
            save_non_closure_factor(var, edges, ratio_3b_2b_w,
                                    ...
                                    exclude_my_bins=TARGET_MY_BINS)
