...
with open(log_filename1, "w") as f_log1:
    with open(log_filename2, "w") as f_log2:
        for var in vars_to_plot:
            ...
            ax.fill_between(edges, band_low_step, band_high_step, step='post', color='gray', alpha=0.3, label="Total Uncertainty")
            ax.fill_between(edges, stat_low_step, stat_high_step, step='post', facecolor='none', edgecolor='green', hatch='////', alpha=0.5, label="Statistical Uncertainty")
            ...
            if var == "Score":
                if args.TrainRegion == "3b":
                    folds_y = get_split_fold_hists(f, var, n_splits=5, n_folds=n_folds, scale_factor=scale_factor)
                    color_list = ["yellow" for i in range(50)]
                    line_alpha = 0.3
                    line_width = 0.5
                else:
                ...
                for i, y_fold in enumerate(folds_y):
                    hep.histplot(y_fold, bins=edges, ax=ax5w, color=color_list[i], linewidth=line_width, linestyle='--', alpha=line_alpha) # , label=f"2b_w Fold {i+1}")
                ...
                fold_errors_2bw = get_fold_errors(f, var, n_folds, TrainRegion=args.TrainRegion) # purely statistical errors from each fold
                    ...
                    if args.TrainRegion == "3b":
                        for j in range(5): 
                            for k in range(1, n_folds+1): 
                                flat_idx = (j * n_folds) + (k - 1)

                                if flat_idx < len(fold_errors_2bw):
                                    val = fold_errors_2bw[flat_idx][i]
                                    f_log2.write(f"Fold {k+1} Split {j} bin {i+1} error: {val}\n")
                    else:
                        for k in range(n_folds):
                            if k < len(fold_errors_2bw):
                                val = fold_errors_2bw[k][i]
                                f_log2.write(f"Fold {k+1} bin {i+1} error: {val}\n")
                ...
                for i, y_fold,  in enumerate(folds_y):
                    ...
                    hep.histplot(ratio_w, bins=edges, ax=rax5w, color=color_list[i], linestyle='--', linewidth=line_width, alpha=line_alpha) # , label=f"Fold {i+1}/Mean")
