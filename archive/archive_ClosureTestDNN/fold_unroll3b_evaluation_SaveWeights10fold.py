...
from fold_functions_ptcut import build_binning_map, processing, fast_fill, get_10fold_filelists
...
parser.add_argument('--isSignal', default=0, type=int, help = "Running on signal samples (for Combine input only)? 0 for background, 1 for signal.")

# parser.add_argument('--foldN', default=0, type=int) # Add for saving the events weights for checking! 
...
if args.runType == "test-only":
    ...
    w_grand_mean_prediction[mask_3T] = current_weights[mask_3T]

    # ==========================================

    n_total_original = aux_data["n_total_events"]
    full_event_weights = np.ones(n_total_original, dtype=np.float32)

    original_indices = aux_data["all_idx"]
    full_event_weights[original_indices] = w_grand_mean_prediction

    weights_filename = f"TransferFactors_{args.YEAR}_{args.TestRegion}.root"
    print(f"Saving ROOT weights Tree to {weights_filename}...")
    
    with uproot.recreate(weights_filename) as f_weight:
        f_weight["Tree_JetInfo"] = {"Transfer_Weight": full_event_weights}
        
    print("Weights Tree successfully saved!")
    # ==========================================
